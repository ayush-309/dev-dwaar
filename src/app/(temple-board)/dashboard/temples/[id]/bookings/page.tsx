...
import { ArrowLeft, Calendar, Clock, MapPin, User, Search, Filter, CheckCircle, XCircle, AlertCircle } from "lucide-react";
...
export default function TempleBookingsPage() {
    ...
    const fetchTempleAndBookings = async () => {
        try {
            // Fetch temple details
            const templeRes = await fetch(`/api/temples/${templeId}`);
            if (templeRes.ok) {
                const templeData = await templeRes.json();
                setTemple(templeData.temple);
            }

            // Fetch bookings for this temple
            const bookingsRes = await fetch(`/api/bookings?templeId=${templeId}`);
            if (bookingsRes.ok) {
        ...
        <div className="space-y-6">
            ...
            <div>
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
                    {temple?.name || "Temple"} Bookings
                </h1>
                <p className="text-gray-600 dark:text-zinc-400 mt-1">
                    {temple ? `${temple.location}, ${temple.city}` : "Manage bookings for this temple"}
                </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                <div className="relative flex-1">
                    ...
                    <input
                        type="text"
                        placeholder="Search by booking number or user..."
                        value={searchTerm}
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-zinc-800">
                                {filteredBookings.map((booking) => (
                                    <tr key={booking.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <p className="font-semibold text-gray-900 dark:text-white">{booking.ticketCount}</p>
                                        </td>
                                        <td className="px-6 py-4">
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-12 text-center">
                    ...
                    <p className="text-gray-500 dark:text-zinc-500">
                        {searchTerm || statusFilter !== "ALL"
                            ...
                            : "No bookings have been made for this temple yet"}
                    </p>
                </div>
            )}
        </div>
